...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		gsap.utils.toArray('.list-rotator-wrapper').forEach((listRotatorWrapper) => {
			...
			const listRotatorTitles = listRotatorWrapper.querySelectorAll(".list-rotator-title");
			...
			if (!isMobile()) {	
				...
				listItems.forEach(function(item, index) {
					...
					// const translateZ = (parseFloat(fontSize) + parseFloat(lineHeight)) * 3.2;

					const computed = window.getComputedStyle(item);
					const fontSize = parseFloat(computed.fontSize);
					const lineHeight = parseFloat(computed.lineHeight);
					const translateZ = (fontSize + lineHeight) * 1.9;
				...
				// 	}
				// });
				listRotatorTitles.forEach((listRotatorTitle) => {
					gsap.to(listRotatorTitle, {
						scrollTrigger: {
						trigger: listRotatorTitle,
						start: "top +=0",
				...
				window.updateListItems = updateListItems;
				window.animateVisibleRotator = animateVisibleRotator;
		});
		...
		$('.has-opacity').each(function(){
			...
			$(this).empty();
			for (index = 0; index < total; index ++){
					if(words[index] == "Avant" || words[index] == "Media" || words[index] == "" || words[index] == ""){
						$(this).append($("<strong /> ").text(words[index] + " "));
					}else{
						$(this).append($("<span /> ").text(words[index] + " "));
					}
			}
